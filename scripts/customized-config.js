...
switch(process.argv[2]) {
  ...
  case 'start':
    rewireModule('react-scripts/scripts/start.js', loadCustomizer('./overrides/config-overrides.dev'));
    break;
  // The "build" script is run to produce a production bundle
  case 'build':
    rewireModule('react-scripts/scripts/build.js', loadCustomizer('./overrides/config-overrides.prod'));
    break;
